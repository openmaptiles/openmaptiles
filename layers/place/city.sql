...
CREATE OR REPLACE FUNCTION place.layer_city(bbox geometry, zoom_level int, pixel_width numeric)
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, place city_place, "rank" int, capital int) AS $$
    SELECT osm_id, geometry, name, COALESCE(NULLIF(name_en, ''), name) AS name_en, place, "rank", place.normalize_capital_level(capital) AS capital
    FROM osm_city_point
    WHERE geometry && bbox
    ...
    SELECT osm_id, geometry, name,
        ...
        place.normalize_capital_level(capital) AS capital
    FROM (
