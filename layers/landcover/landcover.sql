...
CREATE OR REPLACE FUNCTION landcover.landcover_class(landuse VARCHAR, "natural" VARCHAR, leisure VARCHAR, wetland VARCHAR) RETURNS TEXT AS $$
    SELECT CASE
...
CREATE OR REPLACE VIEW landcover.landcover_z0 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z2 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z5 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z8 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z9 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z10 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z11 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z12 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z13 AS (
...
CREATE OR REPLACE VIEW landcover.landcover_z14 AS (
...
CREATE OR REPLACE FUNCTION landcover.layer_landcover(bbox geometry, zoom_level int)
RETURNS TABLE(osm_id bigint, geometry geometry, class text, subclass text) AS $$
    SELECT osm_id, geometry,
        landcover.landcover_class(landuse, "natural", leisure, wetland) AS class,
        COALESCE(
        ...
        SELECT * FROM landcover.landcover_z0
        WHERE zoom_level BETWEEN 0 AND 1 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z2 -> layer_landcover:z2_4        
        SELECT * FROM landcover.landcover_z2
        WHERE zoom_level BETWEEN 2 AND 4 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z5 -> layer_landcover:z5_7
        SELECT * FROM landcover.landcover_z5
        WHERE zoom_level BETWEEN 5 AND 7 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z8 -> layer_landcover:z8
        SELECT *
        FROM landcover.landcover_z8 WHERE zoom_level = 8 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z9 -> layer_landcover:z9        
        SELECT *
        FROM landcover.landcover_z9 WHERE zoom_level = 9 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z10 -> layer_landcover:z10
        SELECT *
        FROM landcover.landcover_z10 WHERE zoom_level = 10 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z11 -> layer_landcover:z11        
        SELECT *
        FROM landcover.landcover_z11 WHERE zoom_level = 11 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z12 -> layer_landcover:z12        
        SELECT *
        FROM landcover.landcover_z12 WHERE zoom_level = 12 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z13 -> layer_landcover:z13        
        SELECT *
        FROM landcover.landcover_z13 WHERE zoom_level = 13 AND geometry && bbox
        UNION ALL
        -- etldoc:  landcover_z14 -> layer_landcover:z14_        
        SELECT *
        FROM landcover.landcover_z14 WHERE zoom_level >= 14 AND geometry && bbox
    ) AS zoom_levels;
$$ LANGUAGE SQL IMMUTABLE;

CREATE OR REPLACE FUNCTION landcover.delete() RETURNS VOID AS $$
BEGIN
  DROP SCHEMA IF EXISTS landcover CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen6 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen5 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen4 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen3 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen2 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon_gen1 CASCADE;
  DROP TABLE IF EXISTS osm_landcover_polygon CASCADE;
END;
$$ LANGUAGE plpgsql;
