...
CREATE OR REPLACE VIEW boundary.boundary_z0 AS (
...
CREATE OR REPLACE VIEW boundary.boundary_z1 AS (
    SELECT geometry, 2 AS admin_level, false AS disputed, false AS maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z3 AS (
    SELECT geometry, 2 AS admin_level, false AS disputed, false AS maritime
    FROM ne_50m_admin_0_boundary_lines_land
    UNION ALL
    SELECT geometry, 4 AS admin_level, false AS disputed, false AS maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z4 AS (
    SELECT geometry, 2 AS admin_level, false AS disputed, false AS maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z5 AS (
    SELECT geometry, 2 AS admin_level, false AS disputed, false AS maritime
    FROM ne_10m_admin_0_boundary_lines_land
    UNION ALL
    SELECT geometry, 4 AS admin_level, false AS disputed, false AS maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z6 AS (
    SELECT geometry, admin_level, disputed, maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z7 AS (
    SELECT geometry, admin_level, disputed, maritime
...
CREATE OR REPLACE VIEW boundary.boundary_z8 AS (
    SELECT geometry, admin_level, disputed, maritime
...
CREATE OR REPLACE FUNCTION boundary.layer_boundary (bbox geometry, zoom_level int)
RETURNS TABLE(geometry geometry, admin_level int, disputed int, maritime int) AS $$
    SELECT geometry, admin_level, disputed::int, maritime::int FROM (
        ...
        SELECT * FROM boundary.boundary_z0 WHERE geometry && bbox AND zoom_level = 0
        UNION ALL
        -- etldoc: boundary_z1 ->  layer_boundary:z1_2
        SELECT * FROM boundary.boundary_z1 WHERE geometry && bbox AND zoom_level BETWEEN 1 AND 2
        UNION ALL
        -- etldoc: boundary_z3 ->  layer_boundary:z3
        SELECT * FROM boundary.boundary_z3 WHERE geometry && bbox AND zoom_level = 3
        UNION ALL
        -- etldoc: boundary_z4 ->  layer_boundary:z4
        SELECT * FROM boundary.boundary_z4 WHERE geometry && bbox AND zoom_level = 4
        UNION ALL
        -- etldoc: boundary_z5 ->  layer_boundary:z5
        SELECT * FROM boundary.boundary_z5 WHERE geometry && bbox AND zoom_level = 5
        UNION ALL
        -- etldoc: boundary_z6 ->  layer_boundary:z6
        SELECT * FROM boundary.boundary_z6 WHERE geometry && bbox AND zoom_level = 6
        UNION ALL
        -- etldoc: boundary_z7 ->  layer_boundary:z7
        SELECT * FROM boundary.boundary_z7 WHERE geometry && bbox AND zoom_level = 7
        UNION ALL
        -- etldoc: boundary_z8 ->  layer_boundary:z8
        SELECT * FROM boundary.boundary_z8 WHERE geometry && bbox AND zoom_level = 8
        UNION ALL
        -- etldoc: boundary_z9 ->  layer_boundary:z9
        SELECT * FROM boundary.boundary_z9 WHERE geometry && bbox AND zoom_level = 9
        UNION ALL
        -- etldoc: boundary_z10 ->  layer_boundary:z10
        SELECT * FROM boundary.boundary_z10 WHERE geometry && bbox AND zoom_level = 10
        UNION ALL
        -- etldoc: boundary_z11 ->  layer_boundary:z11
        SELECT * FROM boundary.boundary_z11 WHERE geometry && bbox AND zoom_level = 11
        UNION ALL
        -- etldoc: boundary_z12 ->  layer_boundary:z12
        SELECT * FROM boundary.boundary_z12 WHERE geometry && bbox AND zoom_level = 12
        UNION ALL
        -- etldoc: boundary_z13 -> layer_boundary:z13
        SELECT * FROM boundary.boundary_z13 WHERE geometry && bbox AND zoom_level >= 13
    ) AS zoom_levels;
$$ LANGUAGE SQL IMMUTABLE;

CREATE OR REPLACE FUNCTION boundary.delete() RETURNS VOID AS $$
BEGIN
  DROP SCHEMA IF EXISTS boundary CASCADE;

  DROP TABLE IF EXISTS osm_boundary_linestring_gen5 CASCADE;
  DROP TABLE IF EXISTS osm_boundary_linestring_gen4 CASCADE;
  DROP TABLE IF EXISTS osm_boundary_linestring_gen3 CASCADE;
  DROP TABLE IF EXISTS osm_boundary_linestring_gen2 CASCADE;
  DROP TABLE IF EXISTS osm_boundary_linestring_gen1 CASCADE;
  DROP TABLE IF EXISTS osm_boundary_linestring;
END;
$$ LANGUAGE plpgsql;
