...
FROM (
    SELECT DISTINCT ON (hl.osm_id)
        ...
        create_route_hstore(rm1.network, rm1.ref, rm1.name) AS route_1,
        create_route_hstore(rm2.network, rm2.ref, rm2.name) AS route_2,
        create_route_hstore(rm3.network, rm3.ref, rm3.name) AS route_3,
        create_route_hstore(rm4.network, rm4.ref, rm4.name) AS route_4,
        create_route_hstore(rm5.network, rm5.ref, rm5.name) AS route_5,
        create_route_hstore(rm6.network, rm6.ref, rm6.name) AS route_6,
        hl.z_order,
