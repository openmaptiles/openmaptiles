...
FROM (
    SELECT DISTINCT ON (hl.osm_id)
        ...
        NULLIF(rm1.network, '') || '=' || COALESCE(rm1.ref, rm1.short_name, rm1.name, '') AS route_1,
        NULLIF(rm2.network, '') || '=' || COALESCE(rm2.ref, rm2.short_name, rm2.name, '') AS route_2,
        NULLIF(rm3.network, '') || '=' || COALESCE(rm3.ref, rm3.short_name, rm3.name, '') AS route_3,
        NULLIF(rm4.network, '') || '=' || COALESCE(rm4.ref, rm4.short_name, rm4.name, '') AS route_4,
        NULLIF(rm5.network, '') || '=' || COALESCE(rm5.ref, rm5.short_name, rm5.name, '') AS route_5,
        NULLIF(rm6.network, '') || '=' || COALESCE(rm6.ref, rm6.short_name, rm6.name, '') AS route_6,
        hl.z_order,
