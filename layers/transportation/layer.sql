...
RETURNS TABLE(osm_id bigint, geometry geometry, class text, subclass text, ramp int, oneway int, brunnel TEXT) AS $$
    SELECT
		osm_id, geometry,
		CASE
            WHEN highway <> '' THEN to_highway_class(highway)
            WHEN railway <> '' THEN railway_class(railway, service)
        END AS class,
	...
	FROM (
		...
		SELECT
			NULL::bigint AS osm_id, geometry, highway, NULL AS railway, NULL AS service,
		...
		SELECT osm_id, geometry, highway, NULL AS railway, NULL AS service, FALSE AS is_bridge, FALSE AS is_tunnel, FALSE AS is_ford, FALSE AS is_ramp, FALSE AS is_oneway, z_order
