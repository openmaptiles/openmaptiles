...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, name_de text, "natural" text, tags hstore, ele int, ele_ft int, "rank" int) AS $$
   -- etldoc: osm_peak_point -> layer_mountain_peak:z7_
   SELECT osm_id, geometry, name, name_en, name_de, "natural", tags, ele::int, ele_ft::int, rank::int
   FROM (
