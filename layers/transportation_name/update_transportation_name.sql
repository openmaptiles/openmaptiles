...
BEGIN
    ...
    FROM (
        SELECT hl.geometry,
            ...
            NULLIF(rm1.network, '') || '=' || COALESCE(NULLIF(rm1.ref,''), NULLIF(rm1.short_name,''), rm1.name) AS route_1,
            NULLIF(rm2.network, '') || '=' || COALESCE(NULLIF(rm2.ref,''), NULLIF(rm2.short_name,''), rm2.name) AS route_2,
            NULLIF(rm3.network, '') || '=' || COALESCE(NULLIF(rm3.ref,''), NULLIF(rm3.short_name,''), rm3.name) AS route_3,
            NULLIF(rm4.network, '') || '=' || COALESCE(NULLIF(rm4.ref,''), NULLIF(rm4.short_name,''), rm4.name) AS route_4,
            NULLIF(rm5.network, '') || '=' || COALESCE(NULLIF(rm5.ref,''), NULLIF(rm5.short_name,''), rm5.name) AS route_5,
            NULLIF(rm6.network, '') || '=' || COALESCE(NULLIF(rm6.ref,''), NULLIF(rm6.short_name,''), rm6.name) AS route_6,
            hl.z_order,
