...
CREATE OR REPLACE FUNCTION water.water_class(waterway TEXT) RETURNS TEXT AS $$
    SELECT CASE WHEN waterway='' THEN 'lake' ELSE 'river' END;
$$ LANGUAGE SQL IMMUTABLE;

CREATE OR REPLACE VIEW water.water_z0 AS (
...
CREATE OR REPLACE VIEW water.water_z1 AS (
...
CREATE OR REPLACE VIEW water.water_z2 AS (
...
CREATE OR REPLACE VIEW water.water_z4 AS (
...
CREATE OR REPLACE VIEW water.water_z5 AS (
...
CREATE OR REPLACE VIEW water.water_z6 AS (
...
CREATE OR REPLACE VIEW water.water_z7 AS (
...
CREATE OR REPLACE VIEW water.water_z8 AS (
...
CREATE OR REPLACE VIEW water.water_z9 AS (
...
CREATE OR REPLACE VIEW water.water_z10 AS (
...
CREATE OR REPLACE VIEW water.water_z11 AS (
...
CREATE OR REPLACE VIEW water.water_z14 AS (
    ...
    SELECT geometry, water.water_class(waterway) AS class FROM osm_water_polygon
);
...
CREATE OR REPLACE FUNCTION water.layer_water (bbox geometry, zoom_level int)
RETURNS TABLE(geometry geometry, class text) AS $$
    SELECT geometry, class::text FROM (
        ...
        SELECT * FROM water.water_z2 WHERE zoom_level BETWEEN 2 AND 3
        UNION ALL
        -- etldoc: water_z4 ->  layer_water:z4        
        SELECT * FROM water.water_z4 WHERE zoom_level = 4
        UNION ALL
        -- etldoc: water_z5 ->  layer_water:z5        
        SELECT * FROM water.water_z5 WHERE zoom_level = 5
        UNION ALL
        -- etldoc: water_z6 ->  layer_water:z6       
        SELECT * FROM water.water_z6 WHERE zoom_level = 6
        UNION ALL
        -- etldoc: water_z7 ->  layer_water:z7        
        SELECT * FROM water.water_z7 WHERE zoom_level = 7
        UNION ALL
        -- etldoc: water_z8 ->  layer_water:z8        
        SELECT * FROM water.water_z8 WHERE zoom_level = 8
        UNION ALL
        -- etldoc: water_z9 ->  layer_water:z9
        SELECT * FROM water.water_z9 WHERE zoom_level = 9
        UNION ALL
        -- etldoc: water_z10 ->  layer_water:z10                   
        SELECT * FROM water.water_z10 WHERE zoom_level = 10
        UNION ALL
        -- etldoc: water_z11 ->  layer_water:z11        
        SELECT * FROM water.water_z11 WHERE zoom_level = 11
        UNION ALL
        -- etldoc: water_z12 ->  layer_water:z2        
        SELECT * FROM water.water_z12 WHERE zoom_level = 12
        UNION ALL
        -- etldoc: water_z13 ->  layer_water:z13        
        SELECT * FROM water.water_z13 WHERE zoom_level = 13
        UNION ALL
        -- etldoc: water_z14 ->  layer_water:z14_        
        SELECT * FROM water.water_z14 WHERE zoom_level >= 14
    ) AS zoom_levels
    WHERE geometry && bbox;
$$ LANGUAGE SQL IMMUTABLE;

CREATE OR REPLACE FUNCTION water.delete() RETURNS VOID AS $$
BEGIN
  DROP SCHEMA IF EXISTS water CASCADE;
  DROP TABLE IF EXISTS osm_water_polygon_gen5 CASCADE;
  DROP TABLE IF EXISTS osm_water_polygon_gen4 CASCADE;
  DROP TABLE IF EXISTS osm_water_polygon_gen3 CASCADE;
  DROP TABLE IF EXISTS osm_water_polygon_gen2 CASCADE;
